...
export default function Home() {
  ...
  const addNote = async (e) => {
    ...
    if (
      newNote.item.body.length > 2 &&
      newNote.item.title.length > 2 &&
      newNote.item.email.length > 2
    ) {
      ...
      const response = await fetch(url, {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          postDetails: newNote.item,
          email: newNote.item.email,
        }),
        credentials: "include",
      });
      const data = await response.json();
    ...
    <section className="flex flex-col items-center w-screen h-screen overflow-scroll gap-8 noscrollbar">
      {getNote ? (
        <article className="flex flex-col gap-8 p-2">
          <h1 className="text-center text-3xl text-blue-500">Note's</h1>
      ...
      {newNote.status && (
        <article className="relative flex flex-col gap-4">
          ...
          <div className="flex flex-col w-full">
            ...
            <input
              defaultValue={newNote.item.email}
              aria-required
              type="text"
              name="addemail"
              id="addemail"
              ...
              onChange={(e) =>
                setNewNote((props) => ({
                  ...props,
                  item: { ...props.item, email: e.target.value },
                }))
              }
            />
          </div>
